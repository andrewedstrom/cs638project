...
class RedlistCriticallyEndangeredHtmlSpider(scrapy.Spider):
    # Unique identifier for spider
    name = 'redlist-critically-endangered-html'

    # Initiate a request and yield an iterable of request
    def start_requests(self):
        urlfile = 'redlist-critically-endangered-misc/urls'
        f = open(urlfile, 'r')
        urls = f.read().split('\n')
        for url in urls:
    ...
    def parse(self, response):
        ...
        filename = 'redlist-critically-endangered-html/%s.html' % animalnum
